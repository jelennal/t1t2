...
from highway2d_layer import HighwayConv2D
from sacred import Experiment
import numpy as np

ex = Experiment('keras_highway_cifar10')
...
@ex.capture
def build_model(act_func, decay, init, std):
    model = Sequential()

    model.add(GaussianNoise(sigma=std, input_shape=(3, 32, 32)))
    model.add(Convolution2D(32, 3, 3, border_mode='same', W_regularizer=l2(decay), init=init))
    #model.add(BatchNormalization(mode=1))
    model.add(GaussianNoise(sigma=std))
    model.add(Activation('relu'))

    model.add(HighwayConv2D(32, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    model.add(GaussianNoise(sigma=std))
    model.add(HighwayConv2D(32, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    model.add(GaussianNoise(sigma=std))
    model.add(HighwayConv2D(32, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    ...
    model.add(Convolution2D(96, 3, 3, border_mode='same', W_regularizer=l2(decay), init=init))
    #model.add(BatchNormalization(mode=1))
    model.add(GaussianNoise(sigma=std))
    model.add(Activation('relu'))

    model.add(HighwayConv2D(96, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    model.add(GaussianNoise(sigma=std))
    model.add(HighwayConv2D(96, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    model.add(GaussianNoise(sigma=std))
    model.add(HighwayConv2D(96, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    ...
    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
    model.add(Convolution2D(128, 3, 3, border_mode='same', W_regularizer=l2(decay), init=init))
    #model.add(BatchNormalization(mode=1))
    model.add(GaussianNoise(sigma=std))
    model.add(Activation('relu'))

    model.add(HighwayConv2D(128, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    model.add(GaussianNoise(sigma=std))
    model.add(HighwayConv2D(128, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    model.add(GaussianNoise(sigma=std))
    model.add(HighwayConv2D(128, 3, 3, init=init, border_mode='same', bT_init=-2, activation=act_func, WT_regularizer=l2(decay), WH_regularizer=l2(decay)))
    ...
    model.add(Convolution2D(10, 1, 1, W_regularizer=l2(decay), init=init))
    model.add(AveragePooling2D(pool_size=(8, 8)))
    #model.add(BatchNormalization(mode=1))
    model.add(GaussianNoise(sigma=std))
    model.add(Flatten())
    model.add(Dense(10, init=init, W_regularizer=l2(decay)))
